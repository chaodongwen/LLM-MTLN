...
def eval_func(doc_couples_all, doc_couples_pred_all):
    ...
    for doc_couples, doc_couples_pred in zip(doc_couples_all, doc_couples_pred_all):
        doc_couples = set([','.join(list(map(lambda x: str(x), doc_couple))) for doc_couple in doc_couples])  # {'5,4', '5,7'}
        doc_couples_pred = set([','.join(list(map(lambda x: str(x), doc_couple))) for doc_couple in doc_couples_pred])  # {'5,4'}

        tmp_num['ec'] += len(doc_couples & doc_couples_pred)  # TP
        tmp_den_p['ec'] += len(doc_couples_pred)  # TP + FP
        tmp_den_r['ec'] += len(doc_couples)  # TP + FN

        doc_emos = set([doc_couple.split(',')[0] for doc_couple in doc_couples])  # {'5'}
        doc_emos_pred = set([doc_couple.split(',')[0] for doc_couple in doc_couples_pred])  # {'5'}
        tmp_num['e'] += len(doc_emos & doc_emos_pred)
        tmp_den_p['e'] += len(doc_emos_pred)
        tmp_den_r['e'] += len(doc_emos)

        doc_caus = set([doc_couple.split(',')[1] for doc_couple in doc_couples])  # {'4', '7'}
        doc_caus_pred = set([doc_couple.split(',')[1] for doc_couple in doc_couples_pred])  # {'4'}
